...
public class Primes {
  ...
  {
    if(!twinPrimeList.contains(pair)) {
...
//Generate and store a list of primes from a given starting point.
  public void generatePrimes(BigInteger candidate, int count)
  {
    ...
    if(count < 1) {
      ...
      //if candidate is even and greater than 2, add one
      if(candidate.remainder(BigInteger.valueOf(2)).equals(BigInteger.ZERO) && candidate.compareTo(BigInteger.valueOf(2)) == 1) {
        candidate = candidate.add(BigInteger.ONE);
      }
      //if candidate is 2, add it to the list and make candidate 3
      else if(candidate.equals(BigInteger.valueOf(2))) {
        primeList.add(candidate);
        candidate = candidate.add(BigInteger.ONE);
        numPrimes = numPrimes + 1;
      }
      //test odd numbers for primality until you have as many prime numbers as count
      while(numPrimes != count) {
        if(AbsoluteTest.isPrime(candidate)) {
          this.addPrime(candidate);
          numPrimes = numPrimes + 1;
        }
        //make candidate the next odd number (even integers cannot be prime)
  ...
  public void generateTwinPrimes()
  {
    ...
    {
      if (primeList.get(i+1).equals((primeList.get(i).add(bigTwo))) )
      {
  ...
  public void generateHexPrimes()
  {
    ...
    for (int i=0; i < twinPrimeList.size()-1; i++)
    {
      ...
      for (int j=i+1; j < twinPrimeList.size(); j++)
      {
        ...
        if (n.multiply(bigTwo).equals(twoN) )
        {
  ...
  {
    //if prime list is not empty
    if(primeList.size() != 0) {
      //convert last prime to string and get length
      String lgstPrime = (primeList.get(primeList.size() - 1)).toString();
      return lgstPrime.length();
    } else {
      return 0;
    }
  }
  ...
  {
    //if cross list is not empty
    if(crossList.size() != 0) {
      Pair<BigInteger> lastCross = crossList.get(crossList.size() - 1);
      
      //get length of left and right hexagon cross
      int l = lastCross.left().toString().length();
      int r = lastCross.right().toString().length();
      Pair<Integer> lastCrossDigits = new Pair<Integer>(l, r);
      
      return lastCrossDigits;
    } else {
  ...
  {
    //implement iterator function that returns iterator
    @Override
  ...
  {
    //implement iterator function that returns iterator LOH
    @Override
